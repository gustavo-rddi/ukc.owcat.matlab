...
clc    ExcelInputFile.Close(false); 
...
inputFile = strcat(data.paths.runDir, filesep, '20200120 - CostReductionScenarios.xlsx');

ExcelInputFile = Excel.Workbooks.Open(inputFile, 0, true); inputFileOpen = true;

outputFile = strcat(data.paths.runDir, filesep, '20200120 OWFC - Market and Site Data.xlsx');
...
% close all
% 
% names = {'Standardisation','Innovation','Central'};
% 
% color = [ [0 128 255]; [118 147 60]; [255 128 0]; [64 64 64]; [64 64 64] ] / 255;
% 
...
%     
%     xlabel('Year of FID', 'FontSize', 12);
%     
%     if i == 1; ylabel('Levelised Cost [€2015/MWh]', 'FontSize', 12); end
%     
%     title(names{i}, 'FontSize', 12);
%     
%     set(h(:),'facecolor', color(i,:));
%     
%     set(gca, 'XTick', year, 'YLim', [0 300], 'Xlim', [2012.5 2032.5]);
%     
% end
% 
% figure('position', [50 75 1350 360]);
...
% 
% LCOEfixMin = sum(LCOE.Best(1:3,:).*fFixed.Best(1:3,:));
% LCOEfixMax = sum(LCOE.Worst(1:3,:).*fFixed.Worst(1:3,:));
% 
% LCOEfloatMin = LCOE.Best(4,:);
% LCOEfloatMax = LCOE.Worst(4,:);
% 
...
%     subplot(1,2,1); hold on; box on;
%     
%     plot([2012 2033], [100 100], 'k:');
%     h(i) = plot(year, LCOEfix, '-o', 'LineWidth', 2);
%     
%     ylabel('Levelised Cost [€2015/MWh]', 'FontSize', 12);
%     xlabel('Year of FID', 'FontSize', 12);
%     
...
% 
% [pFixBest, LRfixBest] = findLRfit(LCOEfixMin, vMarketFix(1:3:12)');
% [pFixWorst, LRfixWorst] = findLRfit(LCOEfixMax, vMarketFix(1:3:12)');
% 
% [pFloatBest, LRfloatBest] = findLRfit(LCOEfloatMin, vMarketFloat(1:3:12)');
% [pFloatWorst, LRfloatWorst] = findLRfit(LCOEfloatMax, vMarketFloat(1:3:12)');
% 
% xFix = linspace(10, 80);
% 
% yFixBest = exp(polyval(pFixBest, log(xFix)));
% yFixWorst = exp(polyval(pFixWorst, log(xFix)));
% 
% xFloat = linspace(eps, 8);
% 
% yFloatBest = exp(polyval(pFloatBest, log(xFloat)));
% yFloatWorst = exp(polyval(pFloatWorst, log(xFloat)));
% 
% figure('position', [150 175 857   360]);
% 
% subplot(1,2,1); hold on; box on;
% patch([xFix, fliplr(xFix)], [yFixBest, fliplr(yFixWorst)], [191 191 191]/256, 'edgecolor', 'none');
% 
% subplot(1,2,2); hold on; box on;
% patch([xFloat, fliplr(xFloat)], [yFloatBest, fliplr(yFloatWorst)], [191 191 191]/256, 'edgecolor', 'none');
% 
% plot([2012 2033], 100*[1 1], 'k:');
% 
% LCOEfixMin = sum(LCOE.Best(1:3,:).*fFixed.Best(1:3,:));
...
% end
% 
% CAPEXfixMin = min(CAPEX.Best(1:3,:));
% CAPEXfixMax = max(CAPEX.Worst(1:3,:));
% 
% CAPEXfloatMin = CAPEX.Best(4,:);
% CAPEXfloatMax = CAPEX.Worst(4,:);
% 
% 

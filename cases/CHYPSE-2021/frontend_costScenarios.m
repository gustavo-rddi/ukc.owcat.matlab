% check if excel servers are currently open, if so, close them
% -------------------------------------------------------------------
...
% make sure all relevant folders are added to the MATLAB path
[~, data] = prepareEnvironment('setup');

% start an excel server to read the inputs from the spreadsheet
...
outputFile = strcat(data.paths.runDir, filesep, '20200120-OWFC-MarketAndSiteData.xlsx');
...
% set cell J17 from the 'Market' spreadsheet to 1
applyMarketSizeChange(ExcelInputFile, 1);

% iterate through sites, scenarios and years
% -------------------------------------------------------------------
% for each of the 4 sites (always 4 sites)?
for i = 1 : numel(site)
%     for each scenario (always the same scenarios)?
    for j = 1 : numel(scenario)
%         for each year (always the same years)?
        for k = 1 : numel(year)
%             for site 'A' and scenario 'Standardisation'
              if i == 1 && j == 1; 
...
% close all excels files (finished with excels)
